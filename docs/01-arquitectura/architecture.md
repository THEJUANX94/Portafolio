...
**Cambio de tema en cascada:** al pulsar el botón de tema, los bloques marcados con el atributo `data-cascade` (tarjeta de perfil, secciones, filas del metro, etc.) cambian de claro a oscuro uno tras otro, de arriba-izquierda hacia abajo. `src/lib/theme-cascade.ts` (función pura `cascadeSteps`, con test en `tests/unit/`) calcula el paso de cada bloque a partir de su posición en pantalla (orden por fila, con tolerancia de 24 px, y luego por columna; los bloques fuera de la ventana van al último paso, máximo 8 pasos). `ThemeToggle.astro` asigna `--cascade-step` a cada bloque y activa la clase `theme-cascade` en `<html>`; mientras esa clase está activa, `global.css` retrasa la transición de color de cada bloque con `transition-delay: calc(var(--cascade-step, 0) * 90ms)`. El leve "respiro" (escala 0.985) de cada bloque se anima con la Web Animations API (`element.animate()`), no con `animation` CSS, para no reiniciar la animación de entrada `rise-in` de las secciones. Al terminar la cascada se quita la clase `theme-cascade` y las variables `--cascade-step`, dejando la página sin transiciones de color fuera de la cascada. Con `prefers-reduced-motion: reduce` el cambio de tema es instantáneo (sin cascada).
